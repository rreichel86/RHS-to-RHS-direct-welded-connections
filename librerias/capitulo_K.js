...
var limites = {
  rama: function (miembro_b, miembro) {
    ...
    if (estaVacio(this.miembros[miembro].seccion) === true || this.miembros[miembro].seccion.nombre === undefined || this.miembros[miembro].seccion.nombre === "Designacion") {
      ...
      if (Fy / Fu <= 0.8 || material === "ASTM A500 Gr. C") {
        ...
        mensaje = "DUCTILIDAD DEL MATERIAL: F<sub>yb</sub>/F<sub>ub</sub> = " + (Fy / Fu).toFixed(3) + " <= 0.8 ó es ASTM A500 Gr. C";
        mensajes.push(mensaje);
      ...
      if (B / t <= 35) {
        ...
        mensaje = "ERROR:  " + nombre + " - ESBELTEZ DE LA PARED: B<sub>b</sub>/t<sub>b</sub> = " + (B / t).toFixed(2) + " es > 35 !";
        return this.Error_set(mensaje);

      }

      if ((Pr <= 0) && (tipo === "CONEXION_EN_K-ESPACIAMIENTO" || tipo === "CONEXION_EN_N-ESPACIAMIENTO" || tipo === "CONEXION_EN_Y" || tipo === "CONEXION_EN_T" || tipo === "CONEXION_EN_X1" || tipo === "CONEXION_EN_X2")) {

        if (B / t <= 1.25 * raiz(E / Fy)) {

          mensaje = "ESBELTEZ DE LA PARED: B<sub>b</sub>/t<sub>b</sub> = " + (B / t).toFixed(2) + " <= 1.25*(E/F<sub>yb</sub>)<sup>0.5</sup> = " + (1.25 * raiz(E / Fy)).toFixed(2);
          mensajes.push(mensaje);

        } else {

          mensaje = "ERROR:  " + nombre + " - ESBELTEZ DE LA PARED: B<sub>b</sub>/t<sub>b</sub> = " + (B / t).toFixed(2) + " > 1.25*(E/F<sub>yb</sub>)<sup>0.5</sup> = " + (1.25 * raiz(E / Fy)).toFixed(2) + " !";
          return this.Error_set(mensaje);
        ...
        if (B / t <= 1.10 * raiz(E / Fy)) {
          ...
          mensaje = "ESBELTEZ DE LA PARED: B<sub>b</sub>/t<sub>b</sub> = " + (B / t).toFixed(2) + " <= 1.10*(E/F<sub>yb</sub>)<sup>0.5</sup> = " + (1.10 * raiz(E / Fy)).toFixed(2);
          mensajes.push(mensaje);

        } else {

          mensaje = "ERROR:  " + nombre + " - ESBELTEZ DE LA PARED: B<sub>b</sub>/t<sub>b</sub> = " + (B / t).toFixed(2) + " > 1.10*(E/F<sub>yb</sub>)<sup>0.5</sup> = " + (1.10 * raiz(E / Fy)).toFixed(2) + " !";
          return this.Error_set(mensaje);
      ...
      if (H / t <= 35) {
        ...
        mensaje = "ERROR:  " + nombre + " - ESBELTEZ DE LA PARED: H<sub>b</sub>/t<sub>b</sub> = " + (H / t).toFixed(2) + " es > 35 !";
        return this.Error_set(mensaje);
      }
      ...
      if ((Pr <= 0) && (tipo === "CONEXION_EN_K-ESPACIAMIENTO" || tipo === "CONEXION_EN_N-ESPACIAMIENTO" || tipo === "CONEXION_EN_Y" || tipo === "CONEXION_EN_T" || tipo === "CONEXION_EN_X1" || tipo === "CONEXION_EN_X2")) {
        ...
        if (H / t <= 1.25 * raiz(E / Fy)) {
          ...
          mensaje = "ESBELTEZ DE LA PARED: H<sub>b</sub>/t<sub>b</sub> = " + (H / t).toFixed(2) + " <= 1.25*(E/F<sub>yb</sub>)<sup>0.5</sup> = " + (1.25 * raiz(E / Fy)).toFixed(2);
          mensajes.push(mensaje);

        } else {

          mensaje = "ERROR:  " + nombre + " - ESBELTEZ DE LA PARED: H<sub>b</sub>/t<sub>b</sub> = " + (H / t).toFixed(2) + " > 1.25*(E/F<sub>yb</sub>)<sup>0.5</sup> = " + (1.25 * raiz(E / Fy)).toFixed(2) + " !";
          return this.Error_set(mensaje);
        ...
        if (H / t <= 1.10 * raiz(E / Fy)) {
          ...
          mensaje = "ESBELTEZ DE LA PARED: H<sub>b</sub>/t<sub>b</sub> = " + (H / t).toFixed(2) + " <= 1.10*(E/F<sub>yb</sub>)<sup>0.5</sup> = " + (1.10 * raiz(E / Fy)).toFixed(2);
          mensajes.push(mensaje);

        } else {

          mensaje = "ERROR:  " + nombre + " - ESBELTEZ DE LA PARED: H<sub>b</sub>/t<sub>b</sub> = " + (H / t).toFixed(2) + " > 1.10*(E/F<sub>yb</sub>)<sup>0.5</sup> = " + (1.10 * raiz(E / Fy)).toFixed(2) + " !";
          return this.Error_set(mensaje);
      ...
      if (tipo === "CONEXION_EN_K-TRASLAPE" || tipo === "CONEXION_EN_N-TRASLAPE" || tipo === "CONEXION_EN_Y" || tipo === "CONEXION_EN_T" || tipo === "CONEXION_EN_X1" || tipo === "CONEXION_EN_X2") {
      ...
      } else if (tipo === "CONEXION_EN_K-ESPACIAMIENTO" || tipo === "CONEXION_EN_N-ESPACIAMIENTO") {
  ...
  cordon: function (miembro) {
    ...
    if (estaVacio(this.miembros[miembro].seccion) === true || this.miembros[miembro].seccion.nombre === undefined || this.miembros[miembro].seccion.nombre === "Designacion") {
      ...
      if (tipo === "CONEXION_EN_K-ESPACIAMIENTO" || tipo === "CONEXION_EN_N-ESPACIAMIENTO" || tipo === "CONEXION_EN_Y" || tipo === "CONEXION_EN_T" || tipo === "CONEXION_EN_X1" || tipo === "CONEXION_EN_X2") {
  ...
  conexion: function (miembro_b1, miembro_b2, miembro) {
    var sin = Math.sin,
      ...
      B = this.miembros[miembro].seccion.B,
      H = this.miembros[miembro].seccion.H,
      betha_eff, Bb_min, Bb_max, mensaje, mensajes = [];
    ...
    if (estaVacio(this.miembros[miembro].seccion) === true || this.miembros[miembro].seccion.nombre === undefined || this.miembros[miembro].seccion.nombre === "Designacion") {
      ...
      if (tipo === "CONEXION_EN_K-ESPACIAMIENTO" || tipo === "CONEXION_EN_N-ESPACIAMIENTO") {
        ...
        if (g >= tb_1 + tb_2) {
          ...
          mensaje = "ESPACIAMIENTO: g = " + (g).toFixed(3) + " >= t<sub>b, 1</sub> + t<sub>b, 2</sub> = " + (tb_1 + tb_2).toFixed(3);
          mensajes.push(mensaje);

        } else {

          mensaje = "ERROR: ESPACIAMIENTO: g = " + (g).toFixed(3) + " < t<sub>b, 1</sub> + t<sub>b, 2</sub> = " + (tb_1 + tb_2).toFixed(3);
          return this.Error_set(mensaje);
...
var resistencia = {
  plantillas: ["<h3>",
                ...
                "<br>",
                "{{#estado}}",
                "P<sub>d</sub> = {{critico._Pd}} kip  >=  P <sub>r</sub> = |{{Pr}}| kip",
                "{{#razon}}",
                "<br>",
                ...
                "<br>",
                "{{#estado}}",
                "M<sub>d-ip</sub> = {{critico._Md}} kip-in  >=  M <sub>r-ip</sub> = |{{Mr_ip}}| kip-in",
                "{{#razon}}",
                "<br>",
                ...
                "<br>",
                "{{#estado}}",
                "M<sub>d-op</sub> = {{critico._Md}} kip-in  >=  M <sub>r-op</sub> = |{{Mr_op}}| kip-in",
                "{{#razon}}",
                "<br>",
  ...
  rama_Y: function (miembro_b, miembro, l_b) {
    var sin = Math.sin,
      abs = Math.abs,
      nombre = this.miembros[miembro_b].nombre.replace(/_|-/g," "),
      Pr = this.miembros[miembro_b].cargas.P,
    ...
    if ((Pr === 0 && Mr_ip === 0 && Mr_op === 0) || (Pr !== 0 && Mr_ip === 0 && Mr_op === 0) || (Pr !== 0 && Mr_ip !== 0 && Mr_op === 0) || (Pr !== 0 && Mr_ip === 0 && Mr_op !== 0) || (Pr !== 0 && Mr_ip !== 0 && Mr_op !== 0)) {
      axial = {};
      axial.resistencias = [];
      if (Bb / B <= 0.85) {
    ...
    if (theta === 90) {
      ...
      if (Pr !== 0 && Mr_ip !== 0 && Mr_op !== 0 && axial.razon !== undefined && flexion_ip.razon !== undefined && flexion_op.razon !== undefined) {
  ...
  rama_X: function (miembro_b, miembro, l_b) {
    var sin = Math.sin,
      cos = Math.cos,
      abs = Math.abs,
      nombre = this.miembros[miembro_b].nombre.replace(/_|-/g," "),
...
var soldadura = {
  tw_min: function (t, tipo) {
    var tipo = tipo || "filete";
  ...
  rama_Y: function (miembro_b, miembro) {
    var abs = Math.abs,
      ...
      b_eoi, le, tw_min, tw_1, tw_2, tw, mensaje = [],
      temp;
    ...
    } else if ((Pr === 0 && Mr_ip === 0 && Mr_op === 0) || theta === 90 && ((Pr !== 0 && Mr_ip !== 0 && Mr_op !== 0) || (Pr !== 0 && Mr_ip !== 0 && Mr_op === 0) || (Pr !== 0 && Mr_ip === 0 && Mr_op !== 0) || (Pr === 0 && Mr_ip !== 0 && Mr_op !== 0))) {
  ...
  rama_Ki: function (miembro_bi, miembro_bj, miembro) {
    var abs = Math.abs,
      ...
      Ov = this.conexion.Ov,
      b_eoi, b_eov,
      le, tw_min, tw_1, tw_2, tw, mensaje = [],
      temp;
    ...
    if (Pr !== 0) {
      ...
      if (theta_i >= 60) {
        mensaje.push("PARA EL PIE DE LA RAMA: ")
        mensaje.push("Usar Soldadura de fillete")
        temp = Math.ceil(tw * 16);
        mensaje.push("con t<sub>w</sub> = " + temp + "/16 in")
      } else if (theta_i < 60) {
        mensaje.push("PARA EL PIE DE LA RAMA: ")
...
var Conexion_K = function (miembro_b1, miembro_b2, miembro) {
  ...
  if ((Pr_1 >= 0 && Pr_2 <= 0) || (Pr_1 <= 0 && Pr_2 >= 0)) {
      ...
      if (tipo === "CONEXION_EN_K" || tipo === "CONEXION_EN_N") {
        ...
        if (limites.rama.apply(this, [miembro_b1, miembro]) !== undefined && limites.rama.apply(this, [miembro_b2, miembro]) !== undefined && limites.cordon.apply(this, [miembro]) !== undefined) {

            if ((Pr_1 === 0 && Pr_2 === 0) || abs((Pr_1 * sin(_theta_1)) / (Pr_2 * sin(_theta_2))) >= 0.8 && abs((Pr_1 * sin(_theta_1)) / (Pr_2 * sin(_theta_2))) <= 1.2) {
              e = 0;
      ...
      if (tipo === "CONEXION_EN_K-ESPACIAMIENTO" || tipo === "CONEXION_EN_N-ESPACIAMIENTO") {
        ...
        if (limites.rama.apply(this, [miembro_b1, miembro]) !== undefined && limites.rama.apply(this, [miembro_b2, miembro]) !== undefined && limites.cordon.apply(this, [miembro]) !== undefined && limites.conexion.apply(this, [miembro_b1, miembro_b2, miembro]) !== undefined) {
...
var Formulas = {
  ...
  Pn_1: function (miembro_b, miembro, betha_eff, Qf_1) {
    ...
    o.Pd = o.phi * o.Pn;
    o._Pn = o.Pn.toFixed(2);
    o._Pd = o.Pd.toFixed(2);
  ...
  Pn_g: function (miembro_b1, miembro_b2, miembro) {
    var abs = Math.abs,
      ...
      h = H - 3 * t,
      Aw = 2 * h * t,
      Cv = Formulas.Cv.apply(thi, [miembro]),
      Vd = 0.9 * 0.6 * Fy * Aw * Cv,
      Pr_1 = this.miembros[miembro_b1].cargas.P,
      theta_1 = this.miembros[miembro_b1].theta,
      _theta_1 = toRad(theta_1),
      ...
      Pi = this.miembros[miembro].cargas.Pi,
      Pd = this.miembros[miembro].cargas.Pd,
      Vr_g = max(abs(Pr_1 * sin(_theta_1)), abs(Pr_2 * sin(_theta_2))),
      Pr_g = max(abs(-Pi - Pr_1 * cos(_theta_1)), abs(Pd + Pr_2 * cos(_theta_2))),
  ...
  le_1: function (miembro_b, mienbro, b_eoi) {
    var sin = Math.sin,
      Hb = this.miembros[miembro_b].seccion.H,
      Bb = this.miembros[miembro_b].seccion.B,
      tb = this.miembros[miembro_b].seccion.tdes,
  ...
  le_2: function (miembro_b) {
    var sin = Math.sin,
      ...
      _theta = toRad(theta),
      le,
      lemiq50 = function (_theta) {
        return 2 * (Hb - 1.2 * tb) / (sin(_theta)) + 2 * (Bb - 1.2 * tb);
      },
      leMiq60 = function (_theta) {
        return 2 * (Hb - 1.2 * tb) / (sin(_theta)) + (Bb - 1.2 * tb);
      };
  ...
  le_3: function (miembro_bi, miembro_bj, miembro, b_eoi, b_eov, Ov) {
    var sin = Math.sin,
      Hb_i = this.miembros[miembro_bi].seccion.H,
      Bb_i = this.miembros[miembro_bi].seccion.B,
      tb_i = this.miembros[miembro_bi].seccion.tdes,
      theta_i = this.miembros[miembro_bi].theta,
      _theta_i = toRad(theta_i),
      Bb_j = this.miembros[miembro_bj].seccion.B,
      tb_j = this.miembros[miembro_bj].seccion.tdes,
      theta_j = this.miembros[miembro_bj].theta,
      _theta_j = toRad(theta_j),
      t = this.miembros[miembro].seccion.tdes,
      b_eoi = b_eoi,
      b_eov = b_eov,
      le;
    ...
    if (Ov >= 25 && Ov < 50) {
      ...
      le = ((2 * Ov) / 50) * ((1 - (Ov / 100)) * (Hb_i / sin(_theta_i)) + (Ov / 100) * (Hb_i / (sin(_theta_i + _theta_j)))) + b_eoi + b_eov;

    } else if (Ov >= 50 && Ov < 80) {

      le = 2 * ((1 - (Ov / 100)) * (Hb_i / sin(_theta_i)) + (Ov / 100) * (Hb_i / (sin(_theta_i + _theta_j)))) + b_eoi + b_eov;

    } else if (Ov >= 80 && Ov <= 100) {

      le = 2 * ((1 - (Ov / 100)) * (Hb_i / sin(_theta_i)) + (Ov / 100) * (Hb_i / (sin(_theta_i + _theta_j)))) + Bb_i + b_eov;

    };

    return le;
  },
  le_4: function (miembro_bj, miembro, b_eoi) {
    var sin = Math.sin,
    ...
    if (Bb_j / B <= 0.85 || theta_j <= 50) {
      ...
      le = ((2 * Hb_j) / (sin(_theta_j))) + 2 * b_eoi;


    } else if (Bb_j / B > 0.85 || theta_j > 50) {

      le = ((2 * Hb_j - 1.2 * tb_j) / (sin(_theta_j)));
    }
  ...
  Mipn_1: function (miembro_b, miembro, Qf_2) {
    ...
    o.Md = o.phi * o.Mn;
    o._Mn = o.Mn.toFixed(2);
    o._Md = o.Md.toFixed(2);
  ...
  Mipn_2: function (miembro_b, miembro) {
    ...
    _Fy = tipo === "CONEXION_EN_T" ? Fy : 0.8 * Fy;

    o = {
      name: "Estado limite: Fluencia local de las paredes laterales del cordon",
      Mn: 0.5 * _Fy * t * potencia((5 * t + Hb), 2),
      phi: 1.00
    };
...
/*  ERRORES
  ...
  -> ERROR: CORDON - ESBELTEZ DE LA PAREDES: B/t es > 35  ó 30 !
    ...
    RAMA-1:
  ----------    
  -> ERROR: RAMA-1 - RESISTENCIA DEL MATERIAL: Fyb es > 52 ksi !
  -> ERROR: RAMA-1 - DUCTILIDAD: Fyb/Fub es >  0.8 ó ASTM 500 GR. C !
  -> ERROR: RAMA-1 - RAZON DE ASPECTO:  Hb/Bb  debe de ser < 0.5 ó > 2 !
  -> ERROR: RAMA-1 - ESBELTEZ DE LA PAREDES: Bb/tb es > 35 ! 
                                             Bb/tb es > 1.25*(E/Fyb)^0.5 = __  ó  1.10*(E/Fyb)^0.5 = __ !
                                             Hb/tb es > 35                               
                                             Hb/tb es > 1.25*(E/Fyb)^0.5 = __  ó  1.10*(E/Fyb)^0.5 = __ !
  -> ERROR: RAMA-1 - ANGULO: theta es < 30 deg ó > 90 deg  !  
  -> ERROR: RAMA-1 - RAZON DE ANCHOS: Bb/B es < 0.25 ó  0.1 + gamma/50 = __ !
                                      Hb/B es < 0.25 ó  0.1 + gamma/50 = __ !  
  ...
  -> ERROR: CORDON: SECCION TRANSVERSAL NO DEFINIDA !
  -> ERROR: CORDON: MATERIAL NO DEFINIDO !
