...
var Control = {
    ...
    reporte_add: function (mensaje, etiqueta) {
         var etiqueta = etiqueta || "p", 
            text = "<"+etiqueta+">";
            ...
            for (i = 0; i < nro; i += 1) {
                $("#" + miembros[i].nombre + "_CARGAS").change(function (event) {
                    var objetivo = $(event.target).prop("id"),
                        indice = objetivo.search("_"),
                        nombre = objetivo.slice(0,indice),
                        propiedades = objetivo.slice(indice+1),
                        indice = Control.indice(nombre);
    ...
    menu: {
        ...
        evento: function () {
            var i, parte = Control.menu.partes,
                nro = parte.length;
